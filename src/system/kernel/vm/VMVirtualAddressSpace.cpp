...
const addr_t VMVirtualAddressSpace::kMaxRandomize			=  0x8000000000ul;
const addr_t VMVirtualAddressSpace::kMaxInitialRandomize	= 0x20000000000ul;
#else
const addr_t VMVirtualAddressSpace::kMaxRandomize			=  0x800000ul;
const addr_t VMVirtualAddressSpace::kMaxInitialRandomize	= 0x2000000ul;
#endif
...
// #pragma mark - VMVirtualAddressSpace


VMVirtualAddressSpace::VMVirtualAddressSpace(team_id id, addr_t base, size_t size)
	:
...
VMVirtualAddressSpace::~VMVirtualAddressSpace()
{
}


inline VMArea*
VMVirtualAddressSpace::FirstArea() const
{
...
inline VMArea*
VMVirtualAddressSpace::NextArea(VMArea* _area) const
{
...
VMArea*
VMVirtualAddressSpace::CreateArea(const char* name, uint32 wiring,
...
void
VMVirtualAddressSpace::DeleteArea(VMArea* _area, uint32 allocationFlags)
{
	VMUserArea* area = static_cast<VMUserArea*>(_area);
	area->~VMUserArea();
...
VMArea*
VMVirtualAddressSpace::LookupArea(addr_t address) const
{
...
//! You must hold the address space's read lock.
VMArea*
VMVirtualAddressSpace::FindClosestArea(addr_t address, bool less) const
{
...
*/
status_t
VMVirtualAddressSpace::InsertArea(VMArea* _area, size_t size,
...
void
VMVirtualAddressSpace::RemoveArea(VMArea* _area, uint32 allocationFlags)
{
	VMUserArea* area = static_cast<VMUserArea*>(_area);
...
bool
VMVirtualAddressSpace::CanResizeArea(VMArea* area, size_t newSize)
{
...
status_t
VMVirtualAddressSpace::ResizeArea(VMArea* _area, size_t newSize,
...
status_t
VMVirtualAddressSpace::ShrinkAreaHead(VMArea* area, size_t size,
	uint32 allocationFlags)
{
...
status_t
VMVirtualAddressSpace::ShrinkAreaTail(VMArea* area, size_t size,
	uint32 allocationFlags)
{
	size_t oldSize = area->Size();
	if (size == oldSize)
...
status_t
VMVirtualAddressSpace::ReserveAddressRange(size_t size,
	const virtual_address_restrictions* addressRestrictions,
...
status_t
VMVirtualAddressSpace::UnreserveAddressRange(addr_t address, size_t size,
	uint32 allocationFlags)
{
	// check to see if this address space has entered DELETE state
	if (fDeleting) {
...
void
VMVirtualAddressSpace::UnreserveAllAddressRanges(uint32 allocationFlags)
{
...
void
VMVirtualAddressSpace::Dump() const
{
	VMAddressSpace::Dump();
	kprintf("area_list:\n");
...
inline bool
VMVirtualAddressSpace::_IsRandomized(uint32 addressSpec) const
{
...
addr_t
VMVirtualAddressSpace::_RandomizeAddress(addr_t start, addr_t end,
...
*/
status_t
VMVirtualAddressSpace::_InsertAreaIntoReservedRegion(addr_t start, size_t size,
...
status_t
VMVirtualAddressSpace::_InsertAreaSlot(addr_t start, addr_t size, addr_t end,
	uint32 addressSpec, size_t alignment, VMUserArea* area,
	uint32 allocationFlags)
{
	TRACE(("VMVirtualAddressSpace::_InsertAreaSlot: address space %p, start "
